...
- [✅][69-x的平方根](Array/easy/69-x的平方根/69-Sqrt(x).js)
#### medium
...
3. 查找类的算法，一般不用虚拟头结点，指针p = head即可；涉及到操作的算法，一般需要虚拟头结点,最后返回dummy.next来记录链表操作后的head
...
### 链表基本操作
```
1. 操作因为边界问题，一般需要虚拟头结点
2. 删除操作：重点是找到要删除的节点的前驱节点
3. 交换操作：一般需要3个指针：pre cur temp
4. 注意指针的步进
```
#### easy
...
## DFS回溯
```
DFS枚举模型 + 判断收集条件 + 去重（usedArray / 排序相邻去重 / 层Set记录用过的index/value）
```
### TODO
...
子集和序列都是对长度不要求的，任意长度
区别是：
子集的相对顺序是不作要求的（所以去重可以用sort+相邻比较continue），也不要求相邻
序列一般是对相对顺序有要求的，所以 一般不能排序, 一般实用Set来对当前层进行去重
```
#### medium
...
2. 在核心的DFS路径枚举算法上，每次加入到path的Str进行判断，判断是否为回文串/合法IP数字，合法就
```
#### medium
