 ...
 var permuteUnique = function(nums) {
  ...
  const backtrack = function(path, usedArry) {
      ...
      for (let i = 0; i < nums.length; i++) {
          // 判重 去重（这里严格意义上叫剪枝，就是当前元素和前面已经DFS过的一样，则当前子树肯定和前面一个元素的子树一样，则就不用DFS当前子树了）：
          // 树层去重, 在排序后重复元素相邻之外 还有一个判断：usedArry[i - 1] === false 这样的话 下面的树枝去重 就无法剪掉这个分支 所以 在这里剪掉；这样条件是为了保留下 某些前面元素已经使用 但是它的分支还未递归尽或者回溯 这样的话 这个分支就是应该遍历的 
          ...
          // 这里的这个去重 就是和46题最大处理不同，结合前面的排序：
            ...
            // 3. 去重还有个前提 必须保证前一个元素是刚刚撤销的 这样的话 证明前一个相邻的相同元素已经枚举过其所有可能了 这次这个相同的元素 就不用枚举了
                ...
                // 所以 这里标记子树用的是下标：
                // usedArr[i] === true 标记的是这个元素在本次枚举路径中使用过
                // usedArr[i - 1] === false 是指i - 1这个元素为root的子树的路径刚刚枚举过，本次就不用再枚举了
          if (i > 0 && nums[i] === nums[i - 1] && usedArry[i - 1] === false) {
            // usedArry[i] = false // me: （其实不加这一句也不影响结果，这是自己加的，更好解释算法）因为之前已经遍历过相同的数字 所以 这里可以标记下为false，,以方便下一个迭代如果还是一样的时候 可以去掉也不影响结果
            // 其实 这里不用加usedArray[i] === false的原因是usedArry是在下面回溯回来的时候，pop的时候 进行了标记 usedArray[i] === false
            continue
          } 
